# mdl-repeat Output Quality Enhancement Proposal — v6

**Changes from v5 (R5 doc fixes)**: Doc-1 (Phase 2 execution order updated to ENG-N2+N10+N12 with 1.5 days; Phase 2 total now 2.5 days; ENG-N11 added to step 6); Doc-2 (ENG-N9.aux removed from execution order, marked completed inline); Doc-3 (audit count corrected: 2+5+1+2 = 10 sites total).

**Changes from v4 (carried)**: integrated R4 reviews. Bio R4 = 0 new items (Bio CONVERGED ✅, R3+R4 both 0). Eng R4 = 1 new item ENG-N12 (`mdl_select_library` has same O(genome_len/8) bitmap scalability issue as ENG-N2; bundle into one patch) + 1 documentation citation correction (call sites of `check_instance_overlap` are 496+532+696+787, not 557+748).

**Changes from v3**: (carried forward in v4) ENG-N11 (`check_instance_overlap` 5 unsafe sites), ENG-N8/N9 spec clarifications, ENG-N9.aux completed inline.
...
The function is called at four merge-gate sites (refine.c:496, 532, 696, 787; corrected from earlier draft's 557+748 which were the `nested_containment_fraction` call sites for B+Q6) to decide whether to block a merge due to instance overlap. Cross-chromosome false positives block desirable merges; false negatives admit incorrect ones.
...
- 2 covered by B+Q6 (lines 225, 230)
- 5 covered by ENG-N11 (lines 268, 275, 284, 307, 311)
...
Total: 8 unsafe sites pre-fix (now all addressed by B/Q6/N8/N11) + 2 safe-by-design.

**Audit COMPLETED in R3. No further audit work needed.**

---

## Tier 1.5b: Big-genome scalability (per Eng-R1)

### ENG-N2 + N10 + N12 (BUNDLED): Eliminate O(genome_len) coverage allocations

Two functions allocate genome-wide coverage arrays of identical scalability class — both must be fixed in one patch:

1. **refine.c:1769** (`refine_prune_families`): `calloc(genome_len, 1)` — 1 byte per base. Wheat (17 Gb) → 17 GB single allocation, fatal.
2. **mdl.c:247-248** (`mdl_select_library`) [NEW R4]: `calloc((genome_len+7)/8, 1)` — bitpacked, 1 bit per base. Wheat → 2.1 GB; maize (2.3 Gb) → 287 MB. Error path (mdl.c:249) silently returns empty result, discarding all accepted families.

**Both replaced with sorted instance intervals + sweep line**, O(num_instances).

**Bundle ALSO with ENG-N10 (R2)**: replace O(n²) selection sort at refine.c:1757-1766 with `qsort`. Same function as ENG-N2, combined patch.

**Severity**: BLOCKER for genomes ≥ 4 Gb (mdl.c silent failure mode is particularly dangerous). REGRESSION test must cover BOTH allocations.

Effort: 1.5 days (was 1 in v4; +0.5 for mdl.c sweep-line + dual-allocation test).
...
## Recommended execution order (revised v6)
...
4. **ENG-N3** (0.5 day): dynamic HASH_SIZE
5. **ENG-N7** (1 hour): -coalesce-factor CLI
   *(ENG-N9.aux audit COMPLETED in R3 — removed from execution order)*

**Phase 2 — Multi-chr + scalability bug fixes (2.5 days, can parallel)**:
6. **B + Q6 + ENG-N11** (1 day): fix all multi-chr unsafe sites in refine.c (`nested_containment_fraction` + `check_instance_overlap`); tighten nested gate predicate; both threading paths
7. **ENG-N8 + ENG-N9** (0.5 day): coalesce + assemble cross-chr fix; bundled patch
8. **ENG-N2 + ENG-N10 + ENG-N12** (1.5 days): refine_prune + mdl_select_library coverage memory (both O(genome_len)) + qsort prune order
...
| ENG-N2+N10+N12 | malloc cap regression test covering BOTH refine.c:1769 AND mdl.c:248 allocations; n_accepted ≥ 10k families to verify qsort scales |
...
| ENG-N11 | Multi-chromosome synthetic; assert merge gate doesn't false-block due to cross-chr false-overlap |
